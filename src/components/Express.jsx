...
const Express = () => {
  ...
  const EasyTestCompleted = localStorage.getItem(`${userPrefix}easyextestCompleted`) === "true";
  const MediumTestCompleted = localStorage.getItem(`${userPrefix}mediumextestCompleted`) === "true";
  const HardTestCompleted = localStorage.getItem(`${userPrefix}hardextestCompleted`) === "true";
  ...
  const handleEasyEx = () => {
    ...
    localStorage.removeItem(`${userPrefix}easyextestCompleted`, "true");
    
   
  };

  const handleMediumEx = () => {
    localStorage.removeItem(`${userPrefix}timeLeft`);
    setTimeLeft(1200); // Reset the timer state to 60 seconds
    ...
    localStorage.removeItem(`${userPrefix}mediumextestCompleted`, "true");
  ...
  const handleHardEx = () => {
    ...
    localStorage.removeItem(`${userPrefix}hardextestCompleted`, "true");
    ...
    <div className="min-h-screen flex items-center justify-center bg-cover bg-center bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500">
      <div className="lg:h-[80vh] lg:w-[90vw] w-[95vw] relative flex flex-col items-center gap-12 justify-center rounded-lg shadow-lg bg-black bg-opacity-30 backdrop-filter backdrop-blur-lg p-8">
        ...
        <div className="flex flex-col md:flex-row gap-8 justify-center items-center">
          ...
          <Link to="/express/easyex" onClick={handleEasyEx}>
          <div className="w-64 h-40 border-4 border-green-500 rounded-lg p-6 flex flex-col justify-center items-center transition-transform transform hover:scale-105 hover:bg-green-200 group">
           ...
           {EasyTestCompleted ? (
              <Link to="/express/easyexsols" onClick={handleExSol}>
                <div className="w-64 h-40 border-4 border-green-500 rounded-lg p-6 flex flex-col justify-center items-center transition-transform transform hover:scale-105 hover:bg-green-200 group">
          ...
          <Link to="/express/mediumex" onClick={handleMediumEx}>
          ...
          {/* {showSolutionLink && ( */}
          {MediumTestCompleted ? (
          <Link to="/express/mediumexsols" onClick={handleExSol}>
          <div className="w-64 h-40 border-4 border-yellow-600 rounded-lg p-6 flex flex-col justify-center items-center transition-transform transform hover:scale-105 hover:bg-yellow-200 group">
          ...
          <Link to="/express/hardex" onClick={handleHardEx}>
          ...
          {/* {showSolutionLink && ( */}
          {HardTestCompleted ? (
          <Link to="/express/hardexsols" onClick={handleExSol}>
