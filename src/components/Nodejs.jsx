...
const Nodejs = () => {
  ...
  const EasyTestCompleted = localStorage.getItem(`${userPrefix}easynodetestCompleted`) === "true";
  const MediumTestCompleted = localStorage.getItem(`${userPrefix}mediumnodetestCompleted`) === "true";
  const HardTestCompleted = localStorage.getItem(`${userPrefix}hardnodetestCompleted`) === "true";


  const handleEasyNode = () => {
    ...
    localStorage.removeItem(`${userPrefix}easynodetestCompleted`, "true");
    
   
  };

  const handleMediumNode = () => {
    localStorage.removeItem(`${userPrefix}timeLeft`);
    setTimeLeft(1200); // Reset the timer state to 60 seconds
    ...
    localStorage.removeItem(`${userPrefix}mediumnodetestCompleted`, "true");
    
   
  };

  const handleHardNode = () => {
    localStorage.removeItem(`${userPrefix}timeLeft`);
    setTimeLeft(1200); // Reset the timer state to 60 seconds
    ...
    localStorage.removeItem(`${userPrefix}hardnodetestCompleted`, "true");
    ...
    <div className="min-h-screen flex items-center justify-center bg-cover bg-center bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500">
      <div className="lg:h-[80vh] lg:w-[90vw] w-[95vw] relative flex flex-col items-center gap-12 justify-center rounded-lg shadow-lg bg-black bg-opacity-30 backdrop-filter backdrop-blur-lg p-8">
        ...
        <div className="flex flex-col md:flex-row gap-8 justify-center items-center">
          ...
          <Link to="/nodejs/easynodejs" onClick={handleEasyNode}>
          ...
          {/* )} */}
           ...
           {EasyTestCompleted ? (
              <Link to="/nodejs/easynodejssols" onClick={handleNodeSol}>
                <div className="w-64 h-40 border-4 border-green-500 rounded-lg p-6 flex flex-col justify-center items-center transition-transform transform hover:scale-105 hover:bg-green-200 group">
          ...
          <Link to="/nodejs/mediumnodejs" onClick={handleMediumNode}>
          ...
          {/* {showSolutionLink && ( */}
          {MediumTestCompleted ? (
          <Link to="/nodejs/mediumnodejssols" onClick={handleNodeSol}>
          <div className="w-64 h-40 border-4 border-yellow-600 rounded-lg p-6 flex flex-col justify-center items-center transition-transform transform hover:scale-105 hover:bg-yellow-200 group">
          ...
          <Link to="/nodejs/hardnodejs" onClick={handleHardNode}>
          ...
          {/* {showSolutionLink && ( */}
          {HardTestCompleted ? (
          <Link to="/nodejs/hardnodejssols" onClick={handleNodeSol}>
